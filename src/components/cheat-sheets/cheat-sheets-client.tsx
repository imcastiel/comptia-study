...
import type { CheatSheet, CheatSheetExam, CheatSheetDomain } from '@/data/cheat-sheets'
...
export function CheatSheetsClient({ sheets }: { sheets: CheatSheet[] }) {
  ...
  const examFiltered = activeExam === 'all'
    ? sheets
    : sheets.filter((s) => s.exam === activeExam)
    ...
    <div className="max-w-3xl mx-auto px-6 py-8">
      ...
      <div className="mb-7">
        ...
        <p className="text-[var(--apple-label-secondary)] text-[14px]">
          {sheets.length} quick-reference sheets — condensed facts, tables, and processes for exam day.
        </p>
      </div>
      ...
      {domains.length > 1 && (
        <div className="flex gap-2 mb-6 flex-wrap">
          ...
          {domains.map((slug) => {
            const sheet = sheets.find((s) => s.domainSlug === slug)!
